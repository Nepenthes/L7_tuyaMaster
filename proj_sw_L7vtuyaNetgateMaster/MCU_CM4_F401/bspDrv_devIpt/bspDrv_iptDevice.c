...
const uint32_t localThreadMainLoopTick = 2;
...
static uint16_t systemRebootDelayCounter = VAR_DISAVAILABLE_U16;

static void devDrvIpt_mainThread(const void *argument);

osThreadId tid_inputDev_process;
osThreadDef(devDrvIpt_mainThread, osPriorityNormal, 1, 2048);
...
usrFunCB_pressShort(void){
	...
	systemRebootDelayCounter = 10000 / localThreadMainLoopTick;
}
...
usrFunCB_pressLongB(void){
	...
	systemRebootDelayCounter = 8000 / localThreadMainLoopTick;
}
...
static void funcLoop_devInputDetectProcess(void){
...
extern void funcLoop_usrTipsProcess(void);
extern void funcLoop_relayActingProcess(void);

static void devDrvIpt_mainThread(const void *argument){

	for(;;){
	
		funcLoop_devInputDetectProcess();
		funcLoop_usrTipsProcess();
		funcLoop_relayActingProcess();
		
		if(VAR_DISAVAILABLE_U16 != systemRebootDelayCounter){
			if(systemRebootDelayCounter)systemRebootDelayCounter --;
			else{
		...
		osDelay(localThreadMainLoopTick);
	}
}
