		...
		else //overflow
		{
			if((uartDataRcv_temp[0] == tuyaF_FRAME_HEAD_A) &&
			   (uartDataRcv_temp[1] == tuyaF_FRAME_HEAD_B)){
				...
				frameLen_temp += tuyaF_FRAMESHELL_DATALEN; 
			}
		}
...
void appFuncProcessTuyaWifiDatapointHandle(uint8_t *dats, uint16_t datsLen){
	...
	uint8_t  dpDn_idLen = dats[0];
	stt_devOpreatDataPonit devDp_keepaccessDataTemp = {0};

	//uint16_t tayaDpDn_dpAllTotelLen = datsLen - (dpDn_idLen + 1);
	uint16_t tayaDpDn_dpAllTotelLen = datsLen - 1; //去掉尾部校验码长度
	...
	/*数据帧头填装*/
	str2Mac5((char *)&dats[3], devMac_temp);
	str2devType((char *)&dats[1], &devType_temp);

	if(!memcmp(devMac_temp, devRootNode_subidMac, 5)){
		
		beeps_usrActive(3, 25, 1); //tips
		tayaDpDn_dataIst += (dpDn_idLen + 1); //单位数据点帧头属性下标索引 --初始化下标
		
		do{
			
			//tayaDpDn_dataIst += (dpDn_idLen + 1); //单位数据点帧头属性下标索引
			memcpy(&tayaDpDn_dataFunc, &dats[tayaDpDn_dataIst], sizeof(struct __sttFunc_tayaDpDn)); //帧头属性值加载
			tayaDpDn_dataIst += sizeof(struct __sttFunc_tayaDpDn); //实际数据下标索引
			tayaDpDn_dataPtr = &dats[tayaDpDn_dataIst]; //实际数据加载
				...
				case SWITCH_TYPE_SWBIT2:{
					...
					switch(tayaDpDn_dataFunc.dataDpDn_dpId){
						...
						case tuyaDP_DEV_SWGANG2_DU_BIT1:{
							...
							swCommand_fromUsr.actMethod = relay_OnOff;
							swCommand_fromUsr.objRelay  = status_actuatorRelay & 0x06 | ((tayaDpDn_dataPtr[0] & 0x01) << 0);
							status_actuatorRelay = swCommand_fromUsr.objRelay;
			...
			case SWITCH_TYPE_SWBIT2:{
				...
				switch(tayaDpDn_dataFunc.dataDpDn_dpId){
					...
					case tuyaDP_DEV_SWGANG2_DU_BIT1:{
						...
						devDp_keepaccessDataTemp.devAgingOpreat_agingReference.agingCmd_swOpreat = 1; //时效置位
						devDp_keepaccessDataTemp.devStatus_Reference.statusRef_swStatus = nodeDev_infoTemp->funcDataMem.swStatus & 0x06 | ((tayaDpDn_dataPtr[0] & 0x01) << 0);
						nodeDev_infoTemp->funcDataMem.swStatus = devDp_keepaccessDataTemp.devStatus_Reference.statusRef_swStatus;
...
//				case msgFunWifi_workmodeGet:{}break;
		...
		if((evt.status == osEventMessage)){
			...
			DBLOG_PRINTF("[TUYA WIFI DBG]tuya uart data rcv[cmd:%02X].\n", rptr_uartWifiData->frameCmd);
...
//					const char *localDev_info = "{\"p\":\""PRODUCT_KEY_GATEWAY"\",\"v\":\""MCU_VER"\",\"m\":0"",\"cap\":7""}";
					const char *localDev_info = "{\"v\":\""MCU_VER"\",\"m\":0"",\"cap\":1""}";
...
//						DBLOG_PRINTF("maxpag get:%d.\n", pagNodeNum_max);
				...
				case tuyaF_CTRL_DPCOME_CMD:{
					...
					if(rptr_uartWifiData->dataKernelLen){
						...
						appFuncProcessTuyaWifiDatapointHandle(rptr_uartWifiData->dataKernel, rptr_uartWifiData->dataKernelLen);
					}
					
				}break;
				
				case tuyaF_DP_GROUP_ADD_CMD_REQ:{
				
					const uint8_t devMacUnitStrLen = 15;
					int groupId = 0;
					uint8_t retCode = 0;
					uint8_t devMacTabTmp[DEV_GROUP_NODE_NUM_MAX][DEV_MAC_LEN] = {0}; 
					char str_gid[16] = {0},
						str_cids[15 * 8] = {0},
						str_rets[16] = {0};
					uint8_t cidsStrlen = 0;
					uint8_t groupDevNodeNum = 0;
					uint8_t loop = 0;
					
//					DBLOG_PRINTF("[GROUP DBG]dats rcv, %s\n", (char *)rptr_uartWifiData->dataKernel);
					sscanf(rptr_uartWifiData->dataKernel, "{\"gid\":\"%[^\"]\",\"cids\":[%[^]]", str_gid, str_cids);
					DBLOG_PRINTF("[GROUP DBG]dats rcv, gid:%s, cids:%s.\n", str_gid, str_cids);
					sscanf(str_gid, "%d", &groupId);
					cidsStrlen = strlen(str_cids) + 2;
					groupDevNodeNum = cidsStrlen / devMacUnitStrLen;
					for(loop = 0; loop < groupDevNodeNum; loop ++){
						appFuncStr2Mac(devMacTabTmp[loop], &str_cids[loop * devMacUnitStrLen + 1]);
						retCode = devGroupOperate_paramAdd(groupId, devMacTabTmp[loop]);
						sprintf(&str_rets[loop * 2], "%d,", retCode);
					}
					str_rets[groupDevNodeNum * 2 - 1] = 0;
					
					uartWifiDataReq_paramTemp.frameCmd = tuyaF_DP_GROUP_ADD_CMD_REQ;
					sprintf((char *)uartWifiDataReq_paramTemp.dataKernel, 
							"{\"gid\":\"%s\",\"cids\":[%s],\"rets\":[%s]}",
							str_gid,
							str_cids,
							str_rets);
					uartWifiDataReq_paramTemp.dataKernelLen = strlen(uartWifiDataReq_paramTemp.dataKernel);
					DBLOG_PRINTF("[GROUP DBG]dats respond, %s.\n", uartWifiDataReq_paramTemp.dataKernel);
					dataReq_trigFlg = true;
					
				}break;
				
				case tuyaF_DP_GROUP_DEL_CMD:{
...
//					DBLOG_PRINTF("[GROUP DBG]dats rcv, %s\n", (char *)rptr_uartWifiData->dataKernel);
					...
					for(loop = 0; loop < groupDevNodeNum; loop ++){
						appFuncStr2Mac(devMacTabTmp[loop], &str_cids[loop * devMacUnitStrLen + 1]);
						retCode = devGroupOperate_paramDel(groupId, devMacTabTmp[loop]);
						sprintf(&str_rets[loop * 2], "%d,", retCode);
					}
					str_rets[groupDevNodeNum * 2 - 1] = 0;
					
					uartWifiDataReq_paramTemp.frameCmd = tuyaF_DP_GROUP_DEL_CMD;
					sprintf((char *)uartWifiDataReq_paramTemp.dataKernel, 
							"{\"gid\":\"%s\",\"cids\":[%s],\"rets\":[%s]}",
							str_gid,
							str_cids,
				...
				case tuyaF_DP_GROUP_CTRL_CMD:{
				
					const uint8_t devMacNull[DEV_MAC_LEN] = {0};
					int gid = 0;
					char gidStr[12] = {0};
					uint8_t gidLen = rptr_uartWifiData->dataKernel[0];
					uint8_t *dptr = NULL;
					uint16_t dptrLen = 0;
					uint16_t dpIndex = 0;
					uint8_t groupIndex = 0;
					uint8_t loop = 0;
					stt_usrDats_devGroupUnitParam *groupParam = systemGroupParamGet();
					
					dpIndex ++;
					memcpy(gidStr, (char *)&rptr_uartWifiData->dataKernel[dpIndex], gidLen);
					dpIndex += gidLen;
					sscanf(gidStr, "%d", &gid);
					dptr = &rptr_uartWifiData->dataKernel[dpIndex];
					dptrLen = rptr_uartWifiData->dataKernelLen - dpIndex;
					
					groupIndex = devGroupOperate_groupIndexGet(gid);
					if(groupIndex < DEV_GROUP_NUM_MAX){
						for(loop = 0; loop < DEV_GROUP_NODE_NUM_MAX; loop ++){
							if(memcmp(devMacNull, groupParam[groupIndex].devNodeMacTab[loop], sizeof(uint8_t) * DEV_MAC_LEN)){ //群组非空设备响应动作
								
								uint8_t dpCtrlTemp[64] = {0};
								uint8_t dpCtrlIndex = 0;
								
								dpCtrlTemp[0] = DEV_MAC_LEN * 2;
								dpCtrlIndex += 1;
								sprintf(&dpCtrlTemp[dpCtrlIndex], 
										"%02X%02X%02X%02X%02X%02X", 
										groupParam[groupIndex].devNodeMacTab[loop][0],
										groupParam[groupIndex].devNodeMacTab[loop][1],
										groupParam[groupIndex].devNodeMacTab[loop][2],
										groupParam[groupIndex].devNodeMacTab[loop][3],
										groupParam[groupIndex].devNodeMacTab[loop][4],
										groupParam[groupIndex].devNodeMacTab[loop][5]);
								dpCtrlIndex += DEV_MAC_LEN * 2;
								memcpy(&dpCtrlTemp[dpCtrlIndex], dptr, dptrLen);
								dpCtrlIndex += dptrLen;
								appFuncProcessTuyaWifiDatapointHandle(dpCtrlTemp, dpCtrlIndex);
								DBLOG_PRINTF("[GROUP DBG]group ctrl excute, group:%d, devIndex:%d.\n", groupIndex, loop);
								osDelay(20);
							}
						}
					}
					
					uartWifiDataReq_paramTemp.frameCmd = tuyaF_DP_GROUP_CTRL_CMD;
					uartWifiDataReq_paramTemp.dataKernelLen = 0;
				...
				case tuyaF_DP_GROUP_CTRL_CHILDS_CMD:{}break;
